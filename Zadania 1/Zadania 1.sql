...
----1.1
SELECT nazwisko,(placa+ISNULL(dod_funkc, 0))*12 AS 'placa roczna'
FROM   Pracownicy;
----1.2
...
----1.3
SELECT nazwa,DATEDIFF(MM,dataRozp,ISNULL(dataZakonczFakt, GETDATE())) AS 'czas trwania'
FROM   Projekty;
----1.4
...
----1.7
SELECT CAST(CAST(2 AS decimal)/CAST(4 AS decimal) AS decimal(2, 2)) AS decimal;
